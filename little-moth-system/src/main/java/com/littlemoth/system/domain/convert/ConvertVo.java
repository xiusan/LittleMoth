...
public class ConvertVo {
    ...
    public static TbSysMenuVo convertVoTbSysMenuVo(TbSysMenu tbSysMenu) {
        ...
        if (Objects.nonNull(tbSysMenu)) {
            ...
            //递归转换
            if(Objects.nonNull(tbSysMenu.getChildren())){
                List<TbSysMenuVo> tbSysMenuVos = convertVoListTbSysMenuVo(tbSysMenu.getChildren());
                tbSysMenuVo.setChildren(tbSysMenuVos);
            }
        }
        return tbSysMenuVo;
    }
     ...
     * 转换为TbTbSysUserVo
     */
    public static TbSysUserInfoVo convertVoTbSysUserInfoVo(TbSysUser user) {
        TbSysUserInfoVo tbSysUserInfoVo = null;
        if (Objects.nonNull(user)) {
            tbSysUserInfoVo = new TbSysUserInfoVo();
            BeanUtils.copyProperties(user, tbSysUserInfoVo);

            tbSysUserInfoVo.setPassword("************");
        }
        return tbSysUserInfoVo;
    }
    /**
     * 资源
